...
export class Hogares implements OnInit {
    ...
    exportExcel() {
        const data = this.hogares().map((h) => ({
            ...
            Manzana: h.manzana ?? '',
            Predio: h.predio ?? '',
            Departamento: h.departamento?.nombre || `ID ${h.departamento_id}`,
            Municipio: h.municipio?.nombre || `ID ${h.municipio_id}`,
            'Centro Poblado': h.centro_poblado?.nombre || (h.centro_poblado_id ? `ID ${h.centro_poblado_id}` : ''),
            Telefono: h.telefono ?? '',
            Estrato: h.estrato ?? '',
